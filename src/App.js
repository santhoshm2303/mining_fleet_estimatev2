...
// ─── 1. GLOBAL CONSTANTS (Absolute Top) ───────────────────────────────
...
// ─── 2. ENGINE & HELPERS (Must exist before UI components) ────────────
...
export default function App(){
  ...
  const [otherA,setOtherA]=useState(() => ({ moistureContent:0.052, exchangeRate:0.70, discountRate:0.115, electricityCost:0.1443, dieselCost:0.9102, allInFitterPerYear:182, mannedOperator:133, calendarTime:8760, diggerFleetRoundingThreshold:0.5 }));
  const [formulas,setFormulas]=useState(() => [
    ...
    {key:"digOpxTotal",label:"Total Digger Opex",unit:"AUD",section:"⛏️ DIGGER — Opex",group:"Totals",formula:"smuHrs * (D_dieselElectricityCost+D_maintenanceLabour+D_oilAndCoolant+D_partsComponentsPM05+D_materialsConsumables+D_get+D_cableCost+D_tracks+D_tires+D_fmsLicenseFee+D_batteryReplacement+D_operatorCost)",hl:1,cur:1},
    ...
    {key:"cycleTime",label:"Total Cycle Time",unit:"min",section:"🚛 TRUCK — Cycle",group:"Cycle",formula:"T_spotTimeLoad + T_queueTimeLoad + F_loadTime + T_spotTimeDump + T_queueTimeDump + T_dumpTime + avgLoadedTravelTime + avgUnloadedTravelTime + avgTkphDelay"},
    {key:"productivity",label:"Productivity",unit:"tph",group:"Output",formula:"T_payload / (cycleTime / 60)",hl:1},
    {key:"trkReqR",label:"Trucks Required (rnd)",unit:"#",section:"🚚 TRUCK — Fleet",group:"Fleet",formula:"CEIL(totalRampMined / productivity / calendarHours)",hl:1},
  ...
  const navGroups=[
    ...
    {label:"Setup",items:[{id:"scenarios",label:"Scenarios",icon:"📋"},{id:"schedule",label:"Schedule",icon:"📅"},{id:"formulas",label:"Formulas",icon:"🧮"}]},
    {label:"Results",items:[{id:"results",label:"Results",icon:"📊"},{id:"comparison",label:"Comparison",icon:"⚖️"}]}
  ];
  ...
  return(
    <div style={{minHeight:"100vh",background:P.bg,color:P.tx,fontFamily:ff}}>
      ...
      <div style={{padding:"20px 32px 60px",maxWidth:1600,margin:"0 auto"}}>
        {/* OTHER / GENERAL ASSUMPTIONS PAGE */}
        ...
        {page==="results"&&(<div>
          ...
          {results.length === 0 ? <p style={{color:P.txD}}>No data found. Check your schedule.</p> : (
              ...
              <div key={grp.key} style={{marginBottom:28}}>
                ...
                <div style={{...cardS,borderTopLeftRadius:0,borderTopRightRadius:0,overflowX:"auto"}}><table style={{borderCollapse:"collapse",fontFamily:ff,fontSize:12,width:"100%",minWidth:600}}>
                  <thead><tr style={{background:P.secBg,borderBottom:`2px solid ${P.bdS}`}}>
                    ...
                    {grp.periods.map((r,i)=><th key={i} style={{...thS,textAlign:"right",color:P.pri,fontWeight:700,minWidth:100}}>{r.periodLabel}</th>)}
                  </tr></thead>
                  <tbody>{formulas.map(f=>{
                    if(f.section) return <tr key={f.key}><td colSpan={2+grp.periods.length} style={{padding:"12px 10px 6px",color:P.pri,fontWeight:700,fontSize:13,background:P.secBg}}>{f.section}</td></tr>;
                    return <tr key={f.key} style={{borderBottom:`1px solid ${P.bd}`}}>
                      ...
                      {grp.periods.map((r,pi)=>{const v=r.res?.[f.key];return<td key={pi} style={{padding:"5px 8px",textAlign:"right",color:P.tx,fontSize:12,fontFamily:mf}}>{f.cur?fmtC2(v):fmt(v,f.dec||2)}</td>})}
                    </tr>})}</tbody>
                </table></div>
              </div>
            ))
        ...
        {/* SCENARIOS & SCHEDULE Placeholder */}
        {page==="scenarios"&&(<div><ST icon="📋">Scenarios</ST></div>)}
        {page==="schedule"&&(<div><ST icon="📅">Schedule</ST></div>)}
