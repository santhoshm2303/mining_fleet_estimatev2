...
export default function App(){
  ...
  const [formulas,setFormulas]=useState(() => [
    ...
    {key:"smuHrs",label:"Digger SMU Hours",unit:"hrs",group:"Digger TUM",formula:"(digHrsReq / digOE) * 1.03"},
    {key:"digQty",label:"Digger Qty per Period",unit:"#",group:"Fleet Sizing",formula:"digHrsReq / (2487 * periodMultiplier)",dec:3},
    ...
    {key:"digOpxTotal",label:"Total Digger Opex",unit:"AUD",section:"⛏️ DIGGER — Opex",group:"Totals",formula:"smuHrs * (D_dieselElectricityCost+D_maintenanceLabour+D_oilAndCoolant+D_partsComponentsPM05+130)",hl:1,cur:1},
    ...
    {key:"totCost",label:"Total Scenario Cost",unit:"AUD",section:"🏆 GRAND TOTAL",group:"Inc Capex",formula:"digCostActivity + (totalRampMined * 2.5)",hl:1,cur:1},
  ...
  return(
    <div style={{minHeight:"100vh",background:P.bg,color:P.tx,fontFamily:ff}}>
      ...
      <div style={{padding:"20px 32px 60px",maxWidth:1600,margin:"0 auto"}}>
        ...
        {/* TRUCKS TAB */}
        {page==="truck"&&(<div>
          <ST icon="🚛">Truck Comparison</ST>
          <div style={{...cardS,overflowX:"auto"}}>
            <table style={{borderCollapse:"collapse",width:"100%"}}>
              <thead><tr style={{background:P.secBg}}><th style={thS}>Parameter</th><th style={thS}>Unit</th>{trucks.map((t,i)=>(<th key={i} style={thS}>{t.truckName}</th>))}</tr></thead>
        ...
        {/* DIGGERS TAB */}
        {page==="digger"&&(<div>
          <ST icon="⛏️">Digger Comparison</ST>
          <div style={{...cardS,overflowX:"auto"}}>
            <table style={{borderCollapse:"collapse",width:"100%"}}>
              <thead><tr style={{background:P.secBg}}><th style={thS}>Parameter</th><th style={thS}>Unit</th>{diggers.map((d,i)=>(<th key={i} style={thS}>{d.diggerName}</th>))}</tr></thead>
        ...
        {page==="results"&&(<div>
          ...
          {results.reduce((acc, r) => {
              const key = `${r.truckName}-${r.diggerName}`;
              if(!acc.find(g => g.key === key)) {
                acc.push({key, truckName: r.truckName, diggerName: r.diggerName, periods: results.filter(x => x.truckName === r.truckName && x.diggerName === r.diggerName)});
              }
              return acc;
            }, []).map(grp => (
